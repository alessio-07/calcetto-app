...
// --- I Componenti DraggablePlayer e DroppableTeam restano uguali ---
function DraggablePlayer({ player, team }) {
  ...
  return (
    <div ref={setNodeRef} style={style} {...listeners} {...attributes} className="bg-white border border-gray-300 p-2 mb-2 rounded shadow-sm touch-none select-none">
...
function DroppableTeam({ id, title, players, bgClass, children }) {
  ...
  return (
    <div ref={setNodeRef} className={`flex-1 p-3 rounded-lg min-h-[300px] ${bgClass}`}>
      ...
      {children}
      {players.map(p => <DraggablePlayer key={p.id} player={p} team={id} />)}
...
export default function MatchCreator() {
  ...
  // NUOVI STATI: Data e Anteprima
  const [matchDate, setMatchDate] = useState(new Date().toISOString().split('T')[0]); // Data di oggi default
  const [isPreview, setIsPreview] = useState(false); // Default è partita giocata
  ...
  async function loadData() {
    ...
    if (id) {
      // Carichiamo anche la DATA e lo STATUS
      ...
      const { data: matchStats } = await supabase.from('match_stats').select('player_id, team, goals, assists, gk_turns, gk_conceded').eq('match_id', id);
  ...
  const saveMatch = async () => {
    ...
    try {
      let matchId = id;
      // Dati da salvare (Punteggio + DATA + STATUS)
      ...
      const statsToInsert = [
        ...teamA.map(p => ({ match_id: matchId, player_id: p.id, team: 'A', ...p.stats })),
        ...teamB.map(p => ({ match_id: matchId, player_id: p.id, team: 'B', ...p.stats }))
      ];
  ...
  return (
    <DndContext sensors={sensors} onDragEnd={handleDragEnd}>
      <div className="p-2 pb-24 max-w-lg mx-auto">
        ...
        {/* TABELLONE (Mostra VS se è preview) */}
        <div className={`flex justify-between items-center text-white p-4 rounded-xl mb-4 shadow-lg sticky top-20 z-40 ${isPreview ? 'bg-gray-600' : 'bg-gray-800'}`}>
          ...
          <div className="text-xl font-bold text-gray-300">
             {isPreview ? 'VS' : (id ? "MODIFICA" : "NUOVA")}
          </div>
        ...
        <div className="flex gap-2 mb-6">
          <DroppableTeam id="teamA" title="Squadra A" players={teamA} bgClass="bg-blue-50 border-2 border-blue-200" />
          <DroppableTeam id="teamB" title="Squadra B" players={teamB} bgClass="bg-red-50 border-2 border-red-200" />
        </div>

        <div className="mb-4">
          <h3 className="font-bold text-gray-700 mb-2">Panchina</h3>
          <DroppableTeam id="pool" title="" players={pool} bgClass="bg-gray-100 border-dashed border-2 border-gray-300 min-h-[100px]" />
        </div>
