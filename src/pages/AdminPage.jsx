...
import { Edit, Trash2, PlusCircle, User, Save, Pencil, Check, X, AlertTriangle, Shield, ShieldAlert } from 'lucide-react';

export default function AdminPage({ session, userRole }) {
  ...
  // Controlli di sicurezza gerarchici
  ...
  // --- LOGICA GESTIONE UTENTI (Solo Founder) ---
  ...
  async function toggleUserRole(userId, currentRole) {
    if (currentRole === 'founder') {
      alert("Non puoi modificare il ruolo del Founder supremo!");
      return;
    }
    const newRole = currentRole === 'admin' ? 'user' : 'admin';
    if (!window.confirm(`Vuoi davvero cambiare il ruolo in ${newRole.toUpperCase()}?`)) return;
    
    await supabase.from('profiles').update({ role: newRole }).eq('id', userId);
    fetchSystemUsers(); // Ricarica la lista aggiornata
  }

  // SCHERMATA DI BLOCCO SE NON SEI ADMIN
  if (!isAdmin) return (
    <div className="w-full min-h-screen bg-slate-900 text-slate-100">
      <div className="flex items-center justify-center p-4 pt-20">
        <div className="bg-red-500/10 border border-red-500/50 p-6 rounded-2xl text-center max-w-sm">
          ...
          <h2 className="text-xl font-bold font-oswald text-white mb-2 tracking-widest">Accesso Negato</h2>
          <p className="text-sm text-slate-400">Non hai i permessi per visualizzare il pannello di amministrazione.</p>
        </div>
      </div>
    </div>
  );
  ...
  return (
    <div className="w-full min-h-screen bg-slate-900 text-slate-100 pb-24">
      <div className="px-4 max-w-7xl mx-auto">
        ...
        {/* --- SEZIONE GESTIONE UTENTI (Visibile SOLO al Founder) --- */}
        {isFounder && (
          <div className="bg-slate-800/80 p-6 rounded-xl shadow-xl mb-8 border border-indigo-500/30 relative overflow-hidden">
            <div className="absolute top-0 right-0 p-4 opacity-10 pointer-events-none"><Shield size={100} /></div>
            <h2 className="text-lg font-bold mb-4 text-indigo-400 flex items-center gap-2 uppercase tracking-wider font-oswald relative z-10">
              <Shield size={20}/> Gestione Permessi
            </h2>
            <div className="max-h-60 overflow-y-auto space-y-2 pr-1 custom-scrollbar relative z-10">
              {systemUsers.map(user => (
                <div key={user.id} className="flex justify-between items-center p-3 rounded-lg border bg-slate-900 border-slate-700/50">
                  <div className="flex flex-col">
                    <span className="font-medium text-slate-200 text-sm truncate">{user.email}</span>
                    <span className={`text-[10px] uppercase tracking-widest font-bold mt-1 ${user.role === 'founder' ? 'text-yellow-400' : user.role === 'admin' ? 'text-indigo-400' : 'text-slate-500'}`}>
                      Ruolo: {user.role}
                    </span>
                  </div>
                  {user.role !== 'founder' && (
                    <button 
                      ...
                      className={`p-2 rounded-lg transition-colors border text-xs font-bold font-oswald tracking-widest ${user.role === 'admin' ? 'bg-slate-800 text-slate-400 border-slate-600 hover:bg-slate-700 hover:text-white' : 'bg-indigo-600/20 text-indigo-400 border-indigo-500/50 hover:bg-indigo-600 hover:text-white'}`}
                    >
                      {user.role === 'admin' ? 'REVOCA ADMIN' : 'PROMUOVI ADMIN'}
                    </button>
                  )}
                </div>
              ))}
                ...
                <div key={match.id} className="flex justify-between items-center border-b border-slate-700 pb-3 last:border-0">
                  <div>
                    ...
                    <div className="font-bold text-white flex items-center gap-2 text-lg font-oswald">
                      ...
                      {hasIssues && <AlertTriangle size={16} className="text-yellow-500 animate-bounce" title="Incongruenza Dati Rilevata" />}
                    </div>
                  </div>
